...
while ($full = <FIND1>) {
    ...
    chomp $full;
    @token = split(/\//, $full);
    #for ($i = 0; $i <= $#token; $i++) {print "debug: token[$i] = $token[$i]\n";}
    $file = $token[$#token];
    #print "debug: file = $file\n";
    @token_base0 = split(/$dir1\//, $full);
    #for ($i = 0; $i <= $#token_base0; $i++) {print "debug: token_base0[$i] = $token_base0[$i]\n";}
    $base0 = $token_base0[1];
    ...
    #for ($i = 0; $i <= $#token_base; $i++) {print "debug: token_base[$i] = $token_base[$i]\n";}
    $base = $token_base[0];
    ...
    $file_escaped2 =~ s/'/\\'/g;
    $file_escaped2 =~ s/&/\\&/g;
    $file_escaped2 =~ s/\\\./\./g; # unescape periods
    #print "debug: file_escaped2 = $file_escaped2\n";
    $base_escaped = $base;
    $base_escaped =~ s/ /\\ /g;
    $base_escaped =~ s/'/\\'/g;
    #print "debug: base_escaped = $base_escaped\n";
